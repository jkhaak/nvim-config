...
-- -- variable to store the terminal buffer reference
-- local flatten_pending = {}  -- token -> { buf, win }
--
...
--   },
--   hooks = {
--     -- Block whenever g:flatten_wait was set
--     should_block = function(argv)
--       return vim.tbl_contains(argv, "-b")
--         or vim.tbl_contains(argv, "let g:flatten_wait=1")
--         or vim.tbl_contains(argv, "-d")
--     end,
--
--     guest_data = function()
--       return { token = tostring(vim.fn.getpid()) .. "-" .. tostring(vim.uv.hrtime()) }
--     end,
--
--     -- Remember the terminal
--     pre_open = function(opts)
--       local token = opts and opts.data and opts.data.token
--       if not token then return end
--
--       local buf, win = current_terminal()
--
--       if buf then
--         flatten_pending[token] = { buf = buf, win = win }
--       end
--     end,
--
...
--
--       if ft == "gitcommit" or ft == "jj" or ft == "gitrebase" then
...
--       local token = opts and opts.data and opts.data.token
--       local target = token and flatten_pending[token] or nil
--       if token then flatten_pending[token] = nil end
--
--       vim.schedule(function()
--         if target and target.buf and vim.api.nvim_buf_is_valid(target.buf) then
...
--           else
--             vim.cmd("tab sbuffer " .. target.buf)
--           end
--         end
--       end)
--     end,
